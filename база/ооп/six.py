# -*- coding: utf-8 -*-
"""
Created on Thu Apr 23 13:17:49 2020
...
def MakeStepVectorized(uOld, numPoints, h, dt, kappa, uNew):
    ...
    u_s = np.zeros(numPoints)
    for i in range(numPoints):
        m = uOld[i]
        u_s[i] = m * m
    one, two, three = np.eye(numPoints), np.eye(numPoints), np.eye(numPoints)
    one[0][0], one[1][1] = 0, 0
    for i in range(2, numPoints):
        one[i][i] = uOld[i-1]
    u_one = 2 * np.dot(u_s, one)
    two[0][0], two[-1][-1] = 0, 0
    for i in range(1, numPoints - 1):
        two[i][i] = uOld[i+1]
    u_two = -2 * np.dot(u_s, two)
    three[0][0] = 0
    for i in range(1, numPoints):
        three[i][i] = uOld[i-1]
    u_three = np.dot(u_s, three)
    # неявная реализация поокоординатных операций с помощью векторизации
    uNew[0:numPoints] = uOld + courant * (u_one + u_two + u_three) 
    # boundaries
...
dt = 0.1#h**2 / (kappa * 20.) # h**2 / (kappa * 20.)
